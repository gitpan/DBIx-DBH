...
vendor to vendor. Another reason is that the DSN string is compound
...
=item * the database vendor must be supplied

The C<vendor> key is used to supply the database
...
to be used, both from DBI and from the vendor's driver.
